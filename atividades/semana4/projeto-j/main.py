...
def exibir_lista_com_indices(itens, atributo_id, atributo_display):
    for i, item in enumerate(itens):
        print(
            f"{i + 1}. {getattr(item, atributo_display)} (ID: {getattr(item, atributo_id)})")
...
def menu_usuarios(repo_usuarios: RepositorioUsuarios, repo_setores: RepositorioSetores):
    while True:
        ...
        if opcao == "1":
            ...
            id_setor_selecionado = selecionar_opcao_por_lista(
                setores_disponiveis, 'id', 'nome', "Selecione o Setor do Usuário: ")
            ...
            for usuario in repo_usuarios.obter_todos():
                print(
                    f"CPF: {usuario.cpf}, Nome: {usuario.nome}, Email: {usuario.email}, Setor: {usuario.id_setor}")
        elif opcao == "0":
            break
        else:
    ...
    while True:
        ...
        if opcao == "1":
            ...
            for tecnico in repo_tecnicos.obter_todos():
                print(
                    f"ID: {tecnico.id}, Nome: {tecnico.nome}, Email: {tecnico.email}, Especialidade: {tecnico.especialidade}")
        elif opcao == "0":
            break
        else:
            print("Opção inválida.")
...
def menu_setores(repo_setores: RepositorioSetores):
    while True:
        ...
        if opcao == "1":
            ...
            # Manter manual para este campo
            id_gestor = int(input("ID do Gestor: "))
            ...
            for setor in repo_setores.obter_todos():
                print(
                    f"ID: {setor.id}, Nome: {setor.nome}, Gestor: {setor.id_gestor}")
        elif opcao == "0":
            break
        else:
            print("Opção inválida.")
    ...
    while True:
        ...
        if opcao == "1":  # Abrir Chamado
            ...
            if user_role == "admin":  # Admin pode abrir chamado para qualquer usuário
                ...
                cpf_usuario = selecionar_opcao_por_lista(
                    usuarios_disponiveis, 'cpf', 'nome', "Selecione o Usuário que está abrindo o Chamado (CPF): ")
            ...
            setores_disponiveis = repo_setores.obter_todos()
            id_setor_selecionado = selecionar_opcao_por_lista(
                setores_disponiveis, 'id', 'nome', "Selecione o Setor associado ao Chamado: ")
            if id_setor_selecionado is None:
            ...
            try:
                ...
                chamado_aberto = repo_chamados.adicionar(
                    cpf_usuario, titulo, descricao, id_setor_selecionado, prioridade)
            ...
            if user_role == "admin":  # Admin option: Atribuir Técnico
                chamados_abertos = [c for c in repo_chamados.obter_todos(
                ) if c.status != StatusChamado.RESOLVIDO and c.status != StatusChamado.CANCELADO]
                id_chamado_selecionado = selecionar_opcao_por_lista(
                    chamados_abertos, 'id', 'titulo', "Selecione o Chamado para atribuir (ID): ")
                ...
                id_tecnico_selecionado = selecionar_opcao_por_lista(
                    tecnicos_disponiveis, 'id', 'nome', "Selecione o Técnico para atribuir (ID): ")
                ...
                chamados_do_usuario = [
                    c for c in repo_chamados.obter_todos() if c.usuario == user_identity]
                if chamados_do_usuario:
                    for c in chamados_do_usuario:
                        print(
                            f"ID: {c.id}, Título: {c.titulo}, Status: {c.status.value}, Prioridade: {c.prioridade.value}")
                else:
                    print("Você não possui chamados abertos.")

        elif opcao == "3":
            if user_role == "admin":  # Admin option: Atualizar Status
                chamados_para_atualizar = [c for c in repo_chamados.obter_todos(
                ) if c.status != StatusChamado.RESOLVIDO and c.status != StatusChamado.CANCELADO]
                id_chamado_selecionado = selecionar_opcao_por_lista(
                    chamados_para_atualizar, 'id', 'titulo', "Selecione o Chamado para atualizar status (ID): ")
                if id_chamado_selecionado is None:
                    print("Nenhum chamado selecionado. Atualização cancelada.")
                    continue

                status_str = input(
                    "Novo Status (aberto, em atendimento, resolvido, cancelado): ").lower()
                chamado = repo_chamados.buscar_por_id(id_chamado_selecionado)
                if chamado:
                    try:
                ...
                chamados_do_usuario = [
                    c for c in repo_chamados.obter_todos() if c.usuario == user_identity]
                id_chamado_selecionado = selecionar_opcao_por_lista(
                    chamados_do_usuario, 'id', 'titulo', "Selecione o Chamado para verificar status (ID): ")
                if id_chamado_selecionado is None:
                ...
                if chamado and chamado.usuario == user_identity:
                    print(
                        f"Chamado ID: {chamado.id}, Título: {chamado.titulo}, Status: {chamado.status.value}, Prioridade: {chamado.prioridade.value}")
                else:
                    print(
                        "Chamado não encontrado ou você não tem permissão para vê-lo.")
        elif opcao == "4":
            if user_role == "admin":  # Admin option: Listar Todos os Chamados
                ...
                if all_chamados:
                    for c in all_chamados:
                        print(
                            f"ID: {c.id}, Usuário: {c.usuario}, Título: {c.titulo}, Status: {c.status.value}, Prioridade: {c.prioridade.value}, Técnico: {c.id_tecnico if c.id_tecnico else 'N/A'}")
                else:
    ...
    while True:
        ...
        if user_role == "admin":
            if opcao == "1":
                ...
                usuarios_disponiveis = repo_usuarios.obter_todos()
                cpf_usuario_selecionado = selecionar_opcao_por_lista(
                    usuarios_disponiveis, 'cpf', 'nome', "Selecione o Usuário para listar chamados (CPF): ")
                ...
                tecnicos_disponiveis = repo_tecnicos.obter_todos()
                id_tecnico_selecionado = selecionar_opcao_por_lista(
                    tecnicos_disponiveis, 'id', 'nome', "Selecione o Técnico para listar chamados (ID): ")
                if id_tecnico_selecionado is None:
            ...
            if opcao == "1":
                chamados_abertos_do_usuario = [c for c in repo_chamados.obter_todos(
                ) if c.usuario == user_identity and c.status == StatusChamado.ABERTO]
...
def menu_gerenciar_usuarios_acesso(repo_autenticacao: RepositorioAutenticacao):
    while True:
        ...
        if opcao == "1":
            ...
            try:
                repo_autenticacao.adicionar_usuario_acesso(
                    username, password, role, cpf_or_id)
            ...
            if usuarios:
                for user in usuarios:
                    print(
                        f"Usuário: {user['username']}, Função: {user['role']}, CPF/ID: {user['cpf_or_id'] if user['cpf_or_id'] else 'N/A'}")
            else:
    ...
    while True:
        ...
        if user_role == "admin":
            ...
            if opcao == "1":
                ...
                menu_relatorios(repo_chamados, repo_usuarios,
                                repo_tecnicos, user_role, user_identity)
            elif opcao == "6":
